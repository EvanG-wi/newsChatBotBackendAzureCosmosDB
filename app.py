...
from cosmic_works.News_Article_AI_Agent import News_Article_AI_Agent
app = FastAPI()
...
def root():
    ...
    """
    return {"status": "meepy2"}

@app.post("/ai")
#def run_cosmic_works_ai_agent(request: AIRequest):
def run_News_Article_AI_Agent(request: AIRequest):
    """
    Run the NewsArticle AI agent.
    """
    if request.session_id not in agent_pool:
        agent_pool[request.session_id] = News_Article_AI_Agent(request.session_id)
